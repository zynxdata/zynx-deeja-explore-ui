...
FOR UPDATE
TO authenticated
USING (public.has_role(auth.uid(), 'owner') OR id = auth.uid())
WITH CHECK (
  CASE 
    WHEN public.has_role(auth.uid(), 'owner') THEN true
    WHEN id = auth.uid() AND role = OLD.role THEN true -- Users can't change their own role
    ELSE false
  END
);
